quest quest_fbook begin
	state start begin
		function get_info(Vnum)
			info_map = {
			 [1] = { 101,"Perros Salvajes", 30},
			 [2] = { 109,"Jabali Rojo ", 35},
			 [3] = { 105,"Lobos Alfa Azul ", 30},
			 [4] = { 107,"Lobos Alfa Gris ", 30},
			 [5] = { 113,"Osos Marrones ", 35},
			 [6] = { 115,"Tigres Blancos ", 20},
			 [7] = { 301,"Soldados Juram. Blanco ", 20},
			 [8] = { 302,"Arqueras Juram. Blanco ", 20},
			 [9] = { 304,"Comandantes Juram. Blanco ", 30},
						}
			Vnum = tonumber(Vnum)
			return info_map[Vnum]
		end
		when 20355.chat."Abandonar Mision Facil" with pc.getf("quest_fbook","quest_book") == 1 begin
			say_title("Capitan:")
			...
			say("Realmente quiere Abortar la Mision Facil?")
			say("")
		...
		when 50307.use begin
			...
			item.remove()
			local info = quest_fbook.get_info(math.random(1,9))
			local vnum = info[1]
	...
	state go_to_disciple begin
		...
		when 20355.chat."Abandonar Mision Facil" begin
			say_title("Capitan:")
			say("En tiempos de guerra muchas misiones importantes")
			say("deben ser completadas y pocos tienen la destreza")
			say("para finalizarlas con exito!")
	...
	state report begin
		when letter begin
			setskin(NOWINDOW)
			makequestbutton("Mision Facil Completada!")
			q.set_title("Mision Facil Completada!")
			q.start()
		end
		when button or info begin
			say_title("Mision Facil Completada:")
			say("Lograste Completar la Mision con Exito!")
			say("Regresa con el Capitan!")
			say("")
		end
		when 20355.chat."Mision Facil Finalizada" begin
			say_title("Capitan:")
			say("Estupendo! Haz Completado otra Mision Facil!")
			say("En tiempos de guerra muchas misiones importantes")
			say("deben ser completadas y pocos tienen la destreza")
			say("para finalizarlas con exito!")
			...
			say("")
			pc.changegold(math.random(100,155)*10000)
			local k = number(1,5)
			local items = {
				{25040, 3},--Pergamino Bendicion
				{25041, 2},--Tela Fina
				{39008, 3},--Pergamino Exorsismo
				{39030, 3},--Lectura Concentrada
				{71108, 100},--Zumo de Granada
							}
			pc.give_item2(items[k][1],items[k][2])
			pc.give_exp_perc("main", 30, 15)
			pc.setqf("collect_target", 0)
			pc.setqf("collect_count1", 0)
			pc.setqf("collect_count2", 0)
			pc.setqf("quest_book", 0)
