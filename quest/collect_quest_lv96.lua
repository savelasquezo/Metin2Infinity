quest collect_quest_lv96 begin
	...
	state information begin
		...
		when __TARGET__.target.click or	20091.chat."Investigacion" begin
			...
			say_title(string.format("~%s~", mob_name(npc.get_race())))
			say("")
			say("Podrias conseguirme Joyas de la Lealtad?")
			say_item_vnum(30253)
			say("Traeme Joyas de la Lealtad, necesito estas")
	...
	state go_to_disciple begin
		...
		when button or info begin
			...
			say("Necesita 30 Joyas de la Lealtad")
			say("Puedes conseguirlas en Gruta del exilio.")
			say_item_vnum(30253)
			say("Puedes conseguir Joyas de la Lealtad Eliminando")
			say("Integrantes del Clan Setaou en Gruta del Exilio.")
			say_reward(string.format("Hasta Ahora has entregado %s[ENTER]Joyas de la Lealtad. ", pc.getqf("collect_count")))
			say("")
		end
		...
		when 2411.kill or 2412.kill or 2413.kill or 2414.kill begin
			...
			if s == 1 then
				...
				syschat("Has Encontrado una Joya de la Lealtad! ")
			end 
		end
		when 20092.take with item.get_vnum() == 30253 begin--DEBUG
			syschat("Metin2 Lamda Error96")
			if pc.count_item(30253) == 200 then
				raw_script("[TEXT_HORIZONTAL_ALIGN_CENTER]")
				say_title(string.format("~%s~", mob_name(npc.get_race())))
				say("")
				local s = select("Max HP +3000", "Defensa +180", "Ataque +150")
				if 1 == s then
				...
				elseif 2 == s then
					...
					pc.setqf("reward", 2)
				elseif 3 == s then 
					affect.add_collect(apply.ATT_GRADE_BONUS, 150, 60*60*24*365*60)
					pc.setqf("reward", 3)
				end
				clear_letter()
				set_quest_state("collect_quest_lvrw", "run")
				set_state(__complete)
			end
		end
		when 20091.chat."Joyas de la Lealtad" with pc.count_item(30253) >0 begin
			if get_time() > pc.getqf("duration") or pc.getqf("drink_drug")!=0 then
				if pc.count_item(30253) >0 then 
					raw_script("[TEXT_HORIZONTAL_ALIGN_CENTER]")
					say_title(string.format("~%s~", mob_name(npc.get_race())))
					say("")
					say("Oh, Has conseguido una Joya de la Lealtad!")
					...
					if s <= pass_percent  then
						...
						if pc.getqf("collect_count") < 29 then    
							...
							say(string.format("Oh! Esta Joya de la Lealtad es de buena calidad!.[ENTER]Sin embargo, todavia necesito %s mas. ", 30-pc.getqf("collect_count")))
							say("")
							pc.setqf("drink_drug",0)
							return
						end
						say("Has conseguido los 30 Joyas de la Lealtad!")
						...
						say_title(string.format("~%s~", mob_name(npc.get_race())))
						say("")
						say("Hm, este Joya de la Lealtad esta fracturada.")
					...
					say(string.format("Todavia necesito %s Joyas de la Lealtad ", 30-pc.getqf("collect_count")))
					say("")
					return
				end
			else
	...
	state __reward begin
		...
		when button or info begin
			...
			say_title(string.format("~%s~", mob_name(20091)))
			say("")
			say("Como recompensa por las Joyas de la Lealtad")
